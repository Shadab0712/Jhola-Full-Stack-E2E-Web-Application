...
public class SwaggerConfiguration {
	...
	private Predicate<String> postPaths() {
		return or(regex("/api/v1/product/.*"), regex("/api/v1/product/.*"));
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("Product Management API for Front End")
				.description("API reference for product management web application")
